  ...
  return (
    <Section id="experience" title="Work Experience">
      <div className="relative overflow-hidden max-w-6xl mx-auto pb-24">
        ...
        <div className="absolute left-5 md:left-1/2 transform md:-translate-x-1/2 border-l-2 border-gray-700 border-opacity-30 h-full"></div>
        ...
        {EXPERIENCES.map((exp, index) => {
          ...
          return (
            <div
              key={index}
              className={`relative mb-20 flex flex-col md:flex-row justify-between items-center w-full ${
                isOdd ? 'md:flex-row-reverse' : ''
              }`}
            >
              {/* Spacer (only visible on desktop) */}
              <div className="hidden md:block order-1 w-[47%]"></div>

              {/* Timeline circle */}
              <div className="absolute left-5 md:left-1/2 transform md:-translate-x-1/2 z-20 bg-accent shadow-lg w-8 h-8 rounded-full flex items-center justify-center">
                <div className="h-3 w-3 bg-primary rounded-full"></div>
              </div>

              {/* Experience Card */}
              <div
                className={`order-1 bg-secondary rounded-lg shadow-xl w-full md:w-[47%] px-6 md:px-10 py-6 mt-10 md:mt-0 ${
                  isOdd ? 'md:mr-10' : 'md:ml-10'
                }`}
              >
                <h3 className="mb-1 font-bold text-light text-xl">{exp.role}</h3>
                <p className="text-sm font-medium leading-snug tracking-wide text-accent">
